...
//        canvas.drawArc(rectF, if (indeterminateMode) startAngleIndeterminateMode else startAngle, angle, false, foregroundPaint)
    ...
    private fun manageColor() {
        foregroundPaint.shader = createSweepGradient(progressBarColorStart ?: progressBarColor,
                progressBarColorEnd ?: progressBarColor, progressBarColorDirection)
    }

    private fun manageBackgroundProgressBarColor() {
        backgroundPaint.shader = createSweepGradient(
    ...
    private fun createSweepGradient(startColor: Int, endColor: Int, gradientDirection: GradientDirection): SweepGradient {
        val cx = width.toFloat() / 2
        val cy = height.toFloat() / 2
        val matrix = Matrix()
        matrix.setRotate(270.0f, cx, cy)

        val sweepGradient = SweepGradient(cx, cy, startColor, endColor)
