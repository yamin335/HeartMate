...
class BillingHelper(
    context: Context,
    private val skuInAppPurchases: List<String>? = null,
    private val skuSubscriptions: List<String>?,
    private val startConnectionImmediately: Boolean = true,
    private var key: String? = BuildConfig.BASE64_ENCODED_PUBLIC_KEY,
    ...
    companion object {
        private const val TAG = "Billing_status"
    }
    ...
    /**
     * Will return a list [Purchase] object or null
     * if no match found.
     * Note that you need to query for owned purchases using [initQuerySkuDetails] or complete a
     * purchase before, in order for this to be not null.
     */
    ...
    /**
     * Will return a list [SkuDetails] object or null if no match found.
     * Note that you need to query for sku details first using [initQueryOwnedPurchases] in order
     * for this not to be null.
     */
    fun getAllSkuDetails(): List<SkuDetails?> {
        return try {
            skuDetailsList
        } catch (e: Exception) {
            listOf<SkuDetails>()
    ...
    fun initClientConnection(
        querySkuDetailsOnConnected: Boolean = this.querySkuDetailsOnConnected,
    ...
    private fun initQueryOwnedPurchasesForType(
        types: List<String>,
    ...
    private fun initQuerySkuDetailsForType(
        types: List<String>,
        currentTypeIndex: Int,
     ...
     * Will init and handle a call to [BillingClient.launchPriceChangeConfirmationFlow]
     */
    fun launchSubChangeConfirmationFlow(
        activity: Activity,
        skuName: String = "",
        purchaseToken: String
    ) {
//        val skuDetailsForChange = getPurchaseWithSkuName("monthly_29")
        val skuDetails = getSkuDetails(skuName)
        if (billingClient.isReady && skuDetails != null) {
            val updatePlanFlowParams: BillingFlowParams = BillingFlowParams.newBuilder()
                .setSkuDetails(skuDetails)
                .setSubscriptionUpdateParams(
                    SubscriptionUpdateParams.newBuilder()
                        .setOldSkuPurchaseToken(purchaseToken)// purchaseToken can be found in Purchase#getPurchaseToken
